...
const personnels = computed(() => {
    ...
    const advisers = props.advisers.map(adviser => ({ id: adviser.id, label: `${adviser.firstname} ${adviser.middlename} ${adviser.lastname} (Adviser)` }))

    const panels = props.panels.map(panel => ({ id: panel.id, label: `${panel.firstname} ${panel.middlename} ${panel.lastname} (Panel)` }))

    const rcs = props.rcs.map(rc => ({ id: rc.id, label: `${rc.firstname} ${rc.middlename} ${rc.lastname} (RC)` }))
        ...
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-1">
                <FormField label="Document">
                    <FormFilePicker v-model="submissionTwo.request.file"/>
                </FormField>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField label="Send to">
                    <FormControl v-model="submissionTwo.request.submitted_to" :options="personnels"/>
                </FormField>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <FormField label="Notes">
                    <FormControl v-model="submissionTwo.request.notes" type="textarea"/>
                </FormField>
            </div>
