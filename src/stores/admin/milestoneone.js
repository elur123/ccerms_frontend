...
export const useMilestoneOneStore = defineStore('milestoneone', {
  state: () => ({
    list: [],
    request: {
      id: '',
  ...
  actions: {
    fetch() {
      axios.get(`${url}api/onemilestones`).then(res => {
        this.list = res.data
      })
    },
    create() {
      return new Promise((resolve, reject) => {
        ...
        axios.post(`${url}api/onemilestones`, formData, 
        { 
    ...
    update() {
      return new Promise((resolve, reject) => {
        ...
        axios.post(`${url}api/onemilestones/${this.request.id}`, formData,
        { 
          headers: { 'Content-Type': 'multipart/form-data' } 
        }
        ).then(res => {
